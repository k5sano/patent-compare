...
12. 引用箇所・ペースト用セルは厳密なコンパクト記法に統一する。請求項は `CL1,2,3`、段落【0001】は `1`、表は `T1`、ページは `P1`。同種は `,`、種類が変わる時だけ `;`、コメントは `/`、防備録メモは `//` の後ろに書く。`CL` を `0L`、`T` を `1`、`S` を `8`、`G` を `6`、`C` を `0` にしない。`T4/備考` を `T4;/備考` にしない。不明文字は `[判読不明]` のまま残す。
