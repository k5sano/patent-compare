...
def _build_llm_prompt(template: dict, ctx: dict, llm_item_ids: list[str]) -> str:
    ...
    return (
        ...
        f"重要: HTMLタグやMarkdown表現は出力しないでください。箇条書きは通常の日本語テキストにしてください。\n"
        f"根拠箇所を書く場合はコンパクト記法に統一します。請求項は CL1,2,3、段落【0001】は 1、"
        f"表1は T1、ページ1は P1 とし、同種はカンマ、種類が変わる時だけ ;、コメントは / の後ろに書きます。\n\n"
        f"---- 本願の請求項 ----\n{seg_block}\n\n"
        f"---- 明細書本文の抜粋 ----\n{excerpt_block}\n\n"
        f"---- 出力すべき項目 ----\n"
