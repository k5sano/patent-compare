...
def _hongan_excerpt(hongan: dict | None) -> str:
    """本願全文 (請求項 + 全段落 + 全表) を inline。

    本願は通常 数万字 (~50K トークン) で、Read ツール経由より「最初から
    prompt に含める」方が以下の理由で速い:
    - LLM が都度 Read を呼び出す往復が消える (レイテンシ短縮)
    - 「見落とし」が起きない (常に全文が context にある)
    - プロンプトキャッシュも効きやすい (同じ案件内の chat スレッド全体)
    引例は依然として Read ツール経由 (12 件で 1M 文字超なので inline は重い)。
    """
    if not hongan:
        return "(本願データなし)"
    lines = [
    ...
    if hongan.get("applicant"):
        lines.append(f"出願人: {hongan.get('applicant')}")
    if hongan.get("application_date"):
        lines.append(f"出願日: {hongan.get('application_date')}")
    lines.append("")
    ...
    if tables:
        lines.append(f"## 表 (全 {len(tables)} 件)")
...
```
{case_dir}/
├── hongan.json                       # 本願 (★ 全文を下にコンテキスト inline 済 → Read 不要)
...
├── citations/<label>.json            # 引例 (★ 本文は Read で都度取得 — 下の引例 index 参照)
...
**ハイブリッド方針**:
- **本願は inline 済** (請求項・全段落・全表) → Read で hongan.json を読まない (重複読み禁止)
- **引例は Read で都度取得** (12 件で 1M 文字超になりやすいので inline しない)
- 引例 paragraphs から特定段落だけ欲しい時は Grep で `{{"id": "0023"` のような
  パターン検索が効率的
"""
...
- **本願 (請求項・全段落・全表) は下のコンテキストに inline 済**。Read 不要。
- **引例の本文/表が要るときだけ Read / Grep ツールで該当 JSON を取得**。
  推測で答えず実データを取りに行くこと。
