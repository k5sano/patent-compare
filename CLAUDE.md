...
対比表、完成版 Excel、検索報告書への貼付用セル、LLM の `cited_location`
では、引用箇所を必ず以下のコンパクト記法に統一する。自然文の
「請求項1」「段落【0001】」「表1」「1ページ」などをそのまま保存・出力しない。

この節は厳密な記載ルールである。意味が通りそうでも推測補完は最小限にし、
英字・数字・記号を勝手に置き換えない。例示の記号列・コードは 1 文字ずつ
厳密に保持し、全角・半角や記号の形を不必要に変更しない。

- 請求項: `CL1`、複数は `CL1,2,3`、範囲は `CL1-10`
- 段落: 接頭辞なし。`【0001】` は `1`、`【0001】〜【0004】` は `1-4`
- 表: `T1`、複数は `T1,2,3`
- 図: `F1`、複数は `F1,2,3`
- ページ: `P1`、複数は `P1,2,3`、範囲は `P1-4`
- ページ行: `P1G2-4`、段組みが必要なら `P1A2-4` / `P1B2-4` 等
- カラム行: `C4G12-15`
- 同じ種類を並列に書くときは、接頭辞を繰り返さず `,` で続ける。
  例: `CL1,2,3,4`、`P1,2,3,4`。`CL1;CL2` や `P1;P2` は不可。
- 種類が変わるときだけ `;` で区切る。
  例: `CL1-10,12,13,14-15;T1-10`
- コメントは `/` の後ろに書く。例: `CL1-10;T1-10/上位概念のみ`
- `//` 以降は防備録メモであり、外部提出用 Excel や検索報告書には出さない。
- 英字と数字を混同しない。`CL` を `0L`、`T` を `1`、`S` を `8`、
  `G` を `6`、`C` を `0` にしない。
- 記号を勝手に別記号へ変換しない。区切りは `;`、コメントは `/`、
  防備録メモは `//`、範囲指定は `-`、複数要素は `,`。
- 不明な文字は推測で確定せず、`[判読不明]` と出力する。
- `T4/備考` は正しく、`T4;/備考` と補わない。
- ページ番号は `P`、ページ中上下左右欄は `A` `B` `C` `D`、
  左右欄は `R` `L`。コラム指定は `C`、行指定は `G`。
  例: `P1A2-4`、`C4G12`。
- 出力前に `CL / F / T / K / E / S / // / ; / , / - / P / A / B / C / D / R / L / G`
  および `C4G12`、`P1A2-4`、`F1,1a,5C`、`T4/備考` が誤変換されて
  いないか確認する。

## local-ai 利用方針

- 低リスクな要約・整形・下書き・抽出作業は `local-ai` を使ってよい。
- `local-ai` の出力は信用しすぎず、実装判断や検証は Codex / Claude 側が行う。
- `local-ai` が間違っても問題ない用途に限定する。
- 適した用途の例:
    - OCR テキストのノイズ整形、見出し分解、短い要約の下書き
    - 書類種別の粗分類、引用候補・段落番号・公報番号の一次抽出
    - 壊れた JSON や表記ゆれの修復案、対比コメントの短縮案
- 避ける用途の例:
    - 新規性 / 進歩性 / 充足性の最終判断
    - 化学構造・数値範囲・法的効果の実質判断
    - 外部公開・保存・上書きを伴う破壊的操作の判断
