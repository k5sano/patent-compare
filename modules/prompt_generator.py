...
## 上位概念マッチの能動的探索手順 (★ ユーザー実務指示 2026-05-05)

「LLM が自分で上位/下位の関係を判定」だけでは見逃しが多い。本願請求項に
**上位概念用語** (例: シリコーン樹脂、ポリオール、界面活性剤、油剤、増粘剤)
が含まれる場合、以下の **手順を能動的に実行** すること。キーワードヒット
だけに頼らず、**引例本文を読み込む** ことが必要。

### 手順
1. **本願語の下位概念候補を列挙** (脳内辞書を使う)
   - 例: 「シリコーン樹脂」 → MQ 樹脂、T 樹脂、トリメチルシロキシケイ酸、
     ジメチコノール、KF-9909、X-25-9138A、KF-7312J、シリコーンレジン、
     架橋型シリコーン、ポリシロキサン樹脂、シリル化シリカ など
   - 例: 「ポリオール」 → グリセリン、プロピレングリコール、1,3-BG、ソルビトール、
     ポリエチレングリコール、PEG、PG、グリセロール など
   - 例: 「界面活性剤」 → SLS、ポリソルベート、ステアリン酸ナトリウム、
     コカミド DEA、ラウリル硫酸ナトリウム、ノニオン系/アニオン系/カチオン系 各種
   - 例: 「油剤」 → スクワラン、ホホバ油、流動パラフィン、ミリスチン酸イソプロピル、
     セチルアルコール、シア脂、各種エステル油

2. **引例の請求項・実施例・配合表でその下位概念を探す**
   - キーワード検索でヒットしなくても、**引例の本文を読み**、上記の下位概念語が
     登場するか確認
   - 商品名・グレード名 (例: KF-9909、X-25-9138A) や略号 (MQ、T、SLS) も探す
   - 引例の段落番号や表番号を特定して `cited_location` に入れる

3. **見つかった場合は ○ 判定**
   - judgment_reason: `(階層判定: <本願上位> ⊃ <引例下位> [= 別名/INCI/慣用名])`
   - 例: `(階層判定: シリコーン樹脂 ⊃ MQ 樹脂 = トリメチルシロキシケイ酸)`
   - cited_text: 下位概念が登場する **原文そのまま** を抜粋
   - 引例本文に **定義箇所** があれば追加: 「MQ 樹脂とは ... である」のような
     説明があれば、その段落も cited_location / cited_text に含める

4. **同義語・別名チェック** (上位/下位とは別)
   - INCI 名 (例: Polysilicone-1) ⇔ 一般名 (例: 架橋型シリコーン)
   - 商品名 (例: KF-9909) ⇔ 化学名 (例: トリメチルシロキシケイ酸)
   - 略号 (例: PET) ⇔ 全称 (例: ポリエチレンテレフタレート)

5. **何も見つからない → ×** (上位概念チェックも下位概念探索も尽くした明示)

### 失敗パターン (悪い例)
- 「シリコーン樹脂」とキーワード検索だけして「該当なし」 → ×
  ※ 引例実施例の「KF-9909 5%」を見逃している
- 「ポリオール」を探して「無し」 → ×
  ※ 引例「グリセリン 10%」「PG 3%」を見逃している
- 単語の表記ゆれを考慮しない (シリコーンレジン ≠ シリコーン樹脂と判定する)
