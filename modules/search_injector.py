...
def inject_search_results(
    ...
    return (
        prompt_text
        + f"\n\n---\n\n{injection}\n\n"
        + "## 重要: 上記は事前のリアルタイム検索結果です。\n"
        + "候補文献の提示にあたっては、上記の検索結果を最優先の情報源として使用してください。\n"
        + "検索結果に含まれる特許番号・出願人・タイトルは実在が確認されています。\n"
        + "訓練データからの推測のみで候補を挙げる場合は、必ず confidence: \"low (未検証)\" と明記してください。\n"
        + "さらに検索ツールを使って追加の検索を行い、候補の網羅性を高めてください。"
    )
