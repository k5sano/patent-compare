...
{{
  ...
  "_note_jplatpat_syntax": {{
    ...
    "tags": {{
      "/TX": "全文", "/TI": "発明の名称", "/AB": "要約", "/CL": "請求の範囲 (実務推奨)",
      "/SP": "明細書", "/AL": "審査官フリーワード+全文",
      "/FI": "FI分類", "/FT": "Fターム", "/FC": "Fタームテーマコード",
      "/IP": "IPC", "/CP": "CPC",
      "/AP": "出願人/権利者", "/IN": "発明者"
    }},
    "tag_pitfall_combinations": "**/AB+CL のような構造タグの '+' 連結は J-PlatPat ではエラー**になるため使わないこと。複数領域を同時にしぼりたい場合は (キーワード)/AB+(キーワード)/CL の形 (キーワード単位の OR) で書く",
    "fterm_rule": "本願付与Fターム 4C083AC172 のようなフルコードをそのまま /FT に入れるとエラー。先頭5文字はテーマコードなので 4C083/FC とし、Fターム本体は AC17.2/FT とする。4C083では末尾1/2が付加コードで、1=請求項、2=実施例を意味するためドット付き (.1/.2) に変換する。4C083の付加コード未指定コードは AD05/FT では AD05.1/AD05.2 がヒットしないため AD05./FT とする。4F100では末尾A/B/Cが層位置を表す場合があるため、4F100AK01B → 4F100/FC*AK01B/FT のように短縮Fターム側へ残す。",
    "examples": [
      "(ポリエチレン+PE+ポリプロピレン)/CL*(積層+ラミネート+多層)/CL",
      "[(ポリエチレン+PE)/CL*(包装+袋)/TX]*B32B27/00/FI",
      "[クラウド/AB+システム/AB]*[製造方法/CL+装飾部品/CL]",
      "(バリア性+ガスバリア)/CL*(4F100/FC*AK01/FT)",
      "(中間層+接着層)/CL*(4F100/FC*AK01B/FT)",
      "(ポロキサマー+プルロニック)/CL*(4C083/FC*AD05./FT)",
      "(グアニルシステイン)/CL*(4C083/FC*AC17.2/FT)",
    ...
    "pitfalls": [
      ...
      "Fタームはテーマコードと併用する。例: 4C083AC172 → 4C083/FC*AC17.2/FT、4C083AD05 → 4C083/FC*AD05./FT、4F100AK01B → 4F100/FC*AK01B/FT",
